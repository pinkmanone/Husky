package com.husky.library.photo.photo;
...
import com.husky.library.R;
import com.husky.library.photo.photo.util.StatusBarCompat;
import com.husky.library.photo.photo.widget.PickConfig;
import com.husky.library.photo.ucrop.UCrop;
...
public class MediaChoseActivity extends AppCompatActivity {
    ...
    public void starPreview(LinkedHashMap map, String currentImage) {
        if (isneedCrop && !isCropOver) {
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        if (imasgemap.size() < 1) {
            ...
            if (chosemode == PickConfig.MODE_MULTIP_PICK) {
                menu.findItem(R.id.menu_photo_count).setTitle("选择(" + imasgemap.size() + "/" + max_chose_count + ")");
            } else {
                menu.findItem(R.id.menu_photo_count).setTitle("选择(1)");
            }
        }
